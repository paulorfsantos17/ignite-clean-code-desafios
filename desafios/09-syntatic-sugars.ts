...
function sumFirstFiveRatings(ratings) {
  ...
  if (ratingsBool) {
    ...
    const isExistFirstFiveRatings = firstFiveRatings === undefined || firstFiveRatings === null

    if (isExistFirstFiveRatings) return { error: 'there must be at least 5 ratings' }
